...
variable "azure_preexisting_resource_group" {
  type        = bool
  default     = false
  description = "Specifies whether an existing resource group should be used or a new one created for installation."
}
...
variable "nfs_ssh_public_key" {
  type = string
  description = "SSH Public Key for nfs VM"
}

variable "nfs_vm_image" {
  type = string
  description = "Image to be used for nfs"
}
